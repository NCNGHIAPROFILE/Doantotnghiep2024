...
class TicketController extends Controller
{
    ...
    {   
        ...
        if($check){
            $user = auth()->user();
            $tickets = Ticket::where('MaSV', $user->MaSV)->get();
    ...
    public function showUserTicketDetail(Request $request)
    {
        $check = auth()->check();
        if($check){
            $masv = $request->input('showUserTicket');
     ...
     */
    public function UpdateBooupdateAccept(string $id)
    {
        $check = auth()->check();
        if($check){
            $user = auth()->user();
